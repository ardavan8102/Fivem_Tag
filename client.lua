...
RegisterNetEvent('ard:set_admins')
AddEventHandler('ard:set_admins', function(admins)
...
AddEventHandler('esx:playerLoaded', function()
    ESX.TriggerServerCallback('ard:getAdminsPlayers', function(admins)
        currentAdminPlayers = admins
    end)
end)

-- 3D Text Settings (Above Head)
function draw3DText(pos, text, options)
    ...
    local cCoords = GetGameplayCamCoords()
    local dist = #(vector3(cCoords.x, cCoords.y, cCoords.z) - vector3(pos.x, pos.y, pos.z))
    ...
    while true do
        ...
        for k, v in pairs(visibleAdmins) do
            ...
            if playerServerID ~= -1 then
                ...
                -- Active Closed Parts If You Want Test Tag + Name (Not Tested By Me In This Version)
                local label
                --local pName = GetPlayerName(player, -1)
                if Config.TagByPermission then
                    label = Config.PermissionLabels[v.permission]
                else
                    label = Config.GroupLabels[v.group] --and pName
                end
